...
// use crate::lexer::token::{TokenType, Keywords, Delimiter,Endmarker,Literal,Operator};
...
//                 Ok(f) => TokenType::LITERALS(Literal::FLOAT(f)),
//                 Err(_) => return Err(LexerError::invalid_float(number, self.position.line, self.position.column))
//             }
//         } else {
//             match number.parse::<i64>() {
//                 Ok(i) => TokenType::LITERALS(Literal::INTEGER(i)),
//                 Err(_) => return Err(LexerError::invalid_integer(number, self.position.line, self.position.column))
//             }
//         };
//
...
//         let peeked_char = self.peek().cloned();
//         match self.current_char {
//             Some('#') => {
